...
this.TextString = {
  ...
  decode: function(data) {
    var begin = data.offset;
    var firstCode = Octet.decode(data);
    if (firstCode == 127) {
...
this.DateValue = {
   ...
   */
  decode: function(data) {
    var numOrArray = LongInteger.decode(data);
    var seconds;
    if (typeof numOrArray == "number") {
      seconds = numOrArray;
    } else {
      seconds = 0;
      for (var i = 0; i < numOrArray.length; i++) {
...
this.TypeValue = {
   ...
   */
  decode: function(data) {
    var numOrStr = ConstrainedEncoding.decode(data);
    if (typeof numOrStr == "string") {
      return numOrStr.toLowerCase();
    }

    var number = numOrStr;
    var entry = WSP_WELL_KNOWN_CONTENT_TYPES[number];
    if (!entry) {
  ...
  encode: function(data, type) {
    var entry = WSP_WELL_KNOWN_CONTENT_TYPES[type.toLowerCase()];
    if (entry) {
...
this.Parameter = {
  ...
  decodeUntypedParameter: function(data) {
    var name = TokenText.decode(data);
  ...
  encodeTypedParameter: function(data, param) {
    var entry = WSP_WELL_KNOWN_PARAMS[param.name.toLowerCase()];
    if (!entry) {
      throw new NotWellKnownEncodingError(
...
this.WellKnownHeader = {
  ...
  decode: function(data) {
    ...
    var entry = WSP_HEADER_FIELDS[index];
    if (!entry) {
      throw new NotWellKnownEncodingError(
   ...
   */
  encode: function(data, header) {
    var entry = WSP_HEADER_FIELDS[header.name.toLowerCase()];
    if (!entry) {
      throw new NotWellKnownEncodingError(
...
this.FieldName = {
  ...
  decode: function(data) {
    ...
    var number = ShortInteger.decode(data);
    var entry = WSP_HEADER_FIELDS[number];
    if (!entry) {
      throw new NotWellKnownEncodingError(
  ...
  encode: function(data, name) {
    var entry = WSP_HEADER_FIELDS[name.toLowerCase()];
    if (entry) {
      ShortInteger.encode(data, entry.number);
    } else {
...
this.WellKnownCharset = {
  ...
  encode: function(data, value) {
    ...
    data.offset = begin;
    var entry = WSP_WELL_KNOWN_CHARSETS[value.charset.toLowerCase()];
    if (!entry) {
      throw new NotWellKnownEncodingError(
...
this.ContentTypeValue = {
  ...
  decodeMediaType: function(data, end) {
    var media = this.decodeMedia(data);
    var params = Parameter.decodeMultiple(data, end);
  ...
  decodeContentGeneralForm: function(data) {
    ...
    var value = this.decodeMediaType(data, end);
  ...
  encodeMediaType: function(data, value) {
    var entry = WSP_WELL_KNOWN_CONTENT_TYPES[value.media.toLowerCase()];
    if (entry) {
      IntegerValue.encode(data, entry.number);
    } else {
...
this.ApplicationIdValue = {
  ...
  decode: function(data) {
    ...
    var id = numOrArray;
    var entry = OMNA_PUSH_APPLICATION_IDS[id];
    if (!entry) {
      throw new NotWellKnownEncodingError(
...
this.PduHelper = {
  ...
  */
  ...
  parsePushHeaders: function(data, msg) {
    ...
    var headersLen = UintVar.decode(data);
    var headersEnd = data.offset + headersLen;

    var contentType = ContentTypeValue.decode(data);
  ...
  parseMultiPart: function(data) {
    var nEntries = UintVar.decode(data);
    if (!nEntries) {
      return null;
    }

    var parts = new Array(nEntries);
    for (var i = 0; i < nEntries; i++) {
      // Length of the ContentType and Headers fields combined.
      var headersLen = UintVar.decode(data);
      // Length of the Data field
      var contentLen = UintVar.decode(data);

      var headersEnd = data.offset + headersLen;
      var contentEnd = headersEnd + contentLen;
      ...
      try {
        ...
        var octetArray = Octet.decodeMultiple(data, contentEnd);
        var content = null;
        var charset = headers["content-type"].params &&
                      headers["content-type"].params.charset
                    ? headers["content-type"].params.charset.charset
                    : null;

        var mimeType = headers["content-type"].media;
        ...
        if (mimeType) {
          if (mimeType == "application/smil") {
            ...
            var tmpStr = this.decodeStringContent(octetArray, charset);
  ...
  composeMultiPart: function(multiStream, parts) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      var slice1 = data.array.slice(headersLen);
      var slice2 = data.array.slice(0, headersLen);
